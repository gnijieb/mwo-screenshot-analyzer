...
# the x1 offset (from) and x2 offset (to) of e.g. playername in cropped entry
EntryT = namedtuple("Entry", "x1 x2")
PixelT = namedtuple("Pixel", "x y")
ColorT = namedtuple("Color", "r g b")
RectangleT = namedtuple("Rectangle", "x1 y1 x2 y2")
ImageT = namedtuple(
    "Image", "resolution psr psrup psrdown psrnone map mode time result rowx1 rowx2 rowheight rows player mech status score kills assists damage cbills xp")
...
def getplayerdata(img, id, filename):
    ...
    while row < 24:
        ...
        cropplayer = img.crop(
            (CONFIG.resinfo.player.x1, y1, CONFIG.resinfo.player.x2, y2))
        # cropplayer.load()
        rawplayer = pytesseract.image_to_string(
            cropplayer, lang="eng", config="-psm 6")
    ...
    log("Found Player: \"%s\" (searched for \"%s\") with distance=%s" %
        (playerfound, CONFIG.player, repr(dist)))
    if CONFIG.loglevel >= DEBUG:
    ...
    cropmech = img.crop((CONFIG.resinfo.mech.x1, y1,
                         CONFIG.resinfo.mech.x2, y2))
    rawmech = pytesseract.image_to_string(
        cropmech, lang="eng", config="-psm 6")
    ...
    if not mechfound:
        ...
        for mechname in Mechs:
            ...
            if tmp_jaro >= 0.55 and tmp_ratio >= 0.55:
                ...
                debug("Possible jaro/ratio match: %s (%4.4f, %4.4f)" %
                      (mechname, tmp_jaro, tmp_ratio))
    ...
    cropstatus = img.crop((CONFIG.resinfo.status.x1, y1,
                           CONFIG.resinfo.status.x2, y2))
    rawstatus = pytesseract.image_to_string(
        cropstatus, lang="eng", config="-psm 6")
    ...
    cropscore = img.crop((CONFIG.resinfo.score.x1, y1,
                          CONFIG.resinfo.score.x2, y2))
    rawscore = pytesseract.image_to_string(
        cropscore, lang="eng", config="-psm 6")
    try:
        score = int(rawscore)
    except ValueError:
    ...
    cropkills = img.crop((CONFIG.resinfo.kills.x1, y1,
                          CONFIG.resinfo.kills.x2, y2))
    rawkills = pytesseract.image_to_string(
        cropkills, lang="eng", config="-psm 6")
    try:
        kills = int(rawkills)
    except ValueError:
    ...
    cropassists = img.crop(
        (CONFIG.resinfo.assists.x1, y1, CONFIG.resinfo.assists.x2, y2))
    rawassists = pytesseract.image_to_string(
        cropassists, lang="eng", config="-psm 6")
    try:
    ...
    cropdamage = img.crop((CONFIG.resinfo.damage.x1, y1,
                           CONFIG.resinfo.damage.x2, y2))
    rawdamage = pytesseract.image_to_string(
        cropdamage, lang="eng", config="-psm 6")
    try:
...
def loadResolutionInfo(res):
    ...
    CONFIG.resinfo = ImageT(
        resolution,
        # psr pixel and corresponding color values for up/down/equal
        PixelT(int(p.get("PSR", "x")), int(p.get("PSR", "y"))),
        ...
        RectangleT(int(p.get("Map", "x1")), int(p.get("Map", "y1")),
                   int(p.get("Map", "x2")), int(p.get("Map", "y2"))),
        RectangleT(int(p.get("Mode", "x1")), int(p.get("Mode", "y1")),
                   int(p.get("Mode", "x2")), int(p.get("Mode", "y2"))),
        RectangleT(int(p.get("Time", "x1")), int(p.get("Time", "y1")),
                   int(p.get("Time", "x2")), int(p.get("Time", "y2"))),
        RectangleT(int(p.get("Result", "x1")), int(p.get("Result", "y1")), int(
            p.get("Result", "x2")), int(p.get("Result", "y2"))),
        ...
        RectangleT(int(p.get("CBills", "x1")), int(p.get("CBills", "y1")), int(
            p.get("CBills", "x2")), int(p.get("CBills", "y2"))),
        RectangleT(int(p.get("XP", "x1")), int(p.get("XP", "y1")),
                   int(p.get("XP", "x2")), int(p.get("XP", "y2")))
    )
...
def processArgs(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "p:t:m:x:vh", ["player=", "type=", "mode=", "ext=", "verbose", "help"])
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            CONFIG.type = arg
            if arg == "fraps":
                # FRAPS Screenshot filename datetime pattern
                CONFIG.fnpattern = r"\d{4}-\d{2}-\d{2} \d{2}-\d{2}-\d{2}"
                # FRAPS Screenshot filename datetime format
                CONFIG.dtformat = "%Y-%m-%d %H-%M-%S"
            elif arg == "mwo":
                # MWO Screenshot filename datetime pattern
                CONFIG.fnpattern = r"\d{2}\.\d{2}\.\d{4}-\d{2}\.\d{2}\.\d{2}"
                CONFIG.dtformat = "%m.%d.%Y-%H.%M.%S"  # MWO Screenshot filename datetime format
            elif arg == "steam":
                # Steam Screenshot filename datetime pattern
                CONFIG.fnpattern = r"\d{4}\d{2}\d{2}\d{2}\d{2}\d{2}"
...
def main(args):
    ...
    while len(files) != 0:
        (map, mode, gametime, result, mech, status, score,
         kills, assists, damage, cbills, xp, psr) = (None,) * 13
        file1 = None
        file2 = None
        ...
        with Image.open(file1) as img:
            ...
            (result, mech, status, score, kills, assists,
             damage) = getplayerdata(img, id, file1)
        if CONFIG.mode == MODE_MATCHPLAYER or CONFIG.mode == MODE_PLAYERMATCH:
        ...
        if CONFIG.mode == MODE_MATCHONLY:
            debug("%s,%s,%s,%s,%s,%s,%d,%d,%d,%d,%d,%d" % (matchdate, CONFIG.player,
                                                           result, mech, map, mode, status, score, kills, assists, damage, gametime))
        else:
            debug("%s,%s,%s,%s,%s,%s,%d,%d,%d,%d,%d,%d,%d,%s,%d" % (matchdate, CONFIG.player, result,
                                                                    mech, map, mode, status, score, kills, assists, damage, xp, cbills, psr, gametime))

        shutil.move(file1, "./processed")
        shutil.move(file2, "./processed")

        writeCSV(dtnow, matchdate, CONFIG.player, result, mech, map, mode,
                 status, score, kills, assists, damage, xp, cbills, psr, gametime)

        if CONFIG.ext:
            ext(matchdate, CONFIG.player, result, mech, map, mode, status,
                score, kills, assists, damage, xp, cbills, psr, gametime)
